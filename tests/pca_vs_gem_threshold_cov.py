...
for i in range(num_iterations):
    ...
    # run models
    pca_offline()
    pca_anomaly_rate = pca_online(h, pca_alpha, num_trials)
    gem_offline()
    gem_anomaly_rate = gem_online(h, gem_alpha, num_trials)
...
# save log
res_string = "--- PCA vs GEM (threshold) took {time} seconds ---".format(time=(time.time()-start_time))
...
# plotting the points  
plt.plot(h_history, pca_anomaly_history, label="PCA") 
plt.plot(h_history, gem_anomaly_history, label="GEM") 
...
plt.title('Anomalies with increasing threshold')
  
# function to show the plot 
plt.legend() 
plt.savefig("pca_vs_gem_threshold_cov.png")
plt.show()
